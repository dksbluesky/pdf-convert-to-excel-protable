...
【嚴格規則】
1. 在輸出表格之前，請先單獨輸出一行中繼資料，格式為：META###日期###店家或場所名稱###是否為發票或收據。日期請盡量判斷並轉換為 YYYY-MM-DD 格式（只取日期，不需要時間），完全無法判斷時請留空；店家/場所名稱請填入收據上的店名或場所全名；第四個欄位請判斷這份文件是否為「逐項記錄購買商品/服務的發票、收據、訂單明細」，是的話填 yes，如果是報價單、菜單、名片、一般文件、表單或其他非購買明細的文件，請填 no。這一行只會出現一次，且只包含這四個部分，輸出完這一行之後才開始輸出表格本身。
...
【嚴格規則】
1. 在輸出表格之前，請先單獨輸出一行中繼資料，格式為：META###日期###店家或場所名稱###是否為發票或收據。日期請盡量判斷並轉換為 YYYY-MM-DD 格式（只取日期，不需要時間），完全無法判斷時請留空；店家/場所名稱若為非中文，請依規則 5 的格式加上中文翻譯（原文 / 中文）；第四個欄位請判斷這份文件是否為「逐項記錄購買商品/服務的發票、收據、訂單明細」，是的話填 yes，如果是報價單、菜單、名片、一般文件、表單或其他非購買明細的文件，請填 no。這一行只會出現一次，且只包含這四個部分，輸出完這一行之後才開始輸出表格本身。
2. 中繼資料那一行之後，每一欄之間請使用 "###" 作為分隔符號。即使表格只有兩、三欄，也一定要在每一欄之間加上 "###"，絕對不可以把一整列當作一段純文字直接輸出而不分欄。
...
def _extract_invoice(file_bytes: bytes, mime_type: str, translate: bool):
    ...
    date_str, store_str, is_invoice = "", "", True
    if lines[0].startswith("META###"):
        ...
        if len(meta_parts) > 3:
            # Default True (include in Raw Sheet) when this part is missing or
            # unparseable — only an explicit "no" excludes the file, so a
            # parsing hiccup degrades to "treat as invoice" rather than
            # silently dropping legitimate data.
            is_invoice = meta_parts[3].strip().lower() not in ("no", "false", "0", "否")
        lines = lines[1:]
    if not lines:
        raise ValueError("no_data")
    ...
    return pd.DataFrame(rows, columns=headers), date_str, store_str, is_invoice
...
def convert_ai_route():
    ...
    def _run(job):
        ...
        if file_bytes is None:
            return filename, None, "", "", True, mime_type  # reason stored in mime_type slot
        try:
            df, date_str, store_str, is_invoice = _extract_invoice(file_bytes, mime_type, translate)
            return filename, df, date_str, store_str, is_invoice, None
        except Exception as e:
            return filename, None, "", "", True, ("429" if "429" in str(e) else "no_data")

    results, meta, failures = {}, {}, []
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(4, len(jobs))) as ex:
        for filename, df, date_str, store_str, is_invoice, reason in ex.map(_run, jobs):
            if df is not None:
                results[filename] = df
                meta[filename] = (date_str, store_str, is_invoice)
            else:
    ...
    for filename, df in results.items():
        ...
        date_str, store_str, is_invoice = meta[filename]
        # Gemini's own judgment (META line) is the primary signal; a missing
        # amount column is a defensive fallback for when that flag is missing
        # or wrong — a file with no amount-like column can't contribute any
        # meaningful rows to the Raw Sheet regardless.
        if is_invoice and _find_amount_col(df.columns):
            raw_parts.append(_normalize_invoice_rows(df, date_str, store_str))
        else:
            skipped_raw.append(filename)
    ...
    if skipped_raw:
        resp.headers["X-Raw-Sheet-Skipped"] = ";".join(quote(fn) for fn in skipped_raw)
    return resp
