...
【嚴格規則】
1. 每一欄之間，請使用 "###" 作為分隔符號。即使表格只有兩、三欄，也一定要在每一欄之間加上 "###"，絕對不可以把一整列當作一段純文字直接輸出而不分欄。
2. 每一列資料換一行。
3. 第一行必須是表頭，欄位數請以「真正的商品/項目表格」的欄數為準。
4. 表格上方若有店名、地址、營業資訊、桌號、訂單時間、收據編號等與商品項目無關的資訊，這些都不是表格的一部分，請完全忽略，不要把它們當成表格的列輸出。只擷取「有多筆品名/數量/金額之類欄位」的那個表格本身。
5. 若某一列在某欄沒有對應的內容（例如備註列在數量、單價欄沒有資料），該儲存格請留空，不要填入任何文字、符號或占位字元（例如「無」、「-」、「叢」等）。
6. 不要輸出任何 Markdown 標記，只要純文字。
7. 金額請保留千分位符號，不要隨意移除。
8. 若遇到跨頁，請自動合併。
9. 底部若有付款條件、稅金、總計等資訊，請整理在表格最下方的列，並把該資訊文字放在「品名／項目」欄；但「金額」欄請留空，不要填入小計、總計、含稅金額等任何數字，因為這些列是文字說明列，不是逐項商品，重複填入金額會導致與表格上方各項金額加總後重複計算。"""

PROMPT_AI_TRANSLATE = """你是一個專業的雙語資料輸入員。請將這份圖片或 PDF 中的表格轉換為純文字資料，並在同一儲存格內附上中文翻譯。

【嚴格規則】
1. 每一欄之間，請使用 "###" 作為分隔符號。即使表格只有兩、三欄，也一定要在每一欄之間加上 "###"，絕對不可以把一整列當作一段純文字直接輸出而不分欄。
2. 每一列資料換一行，第一行必須是表頭，欄位數請以「真正的商品/項目表格」的欄數為準。
3. 表格上方若有店名、地址、營業資訊、桌號、訂單時間、收據編號等與商品項目無關的資訊，這些都不是表格的一部分，請完全忽略，不要把它們當成表格的列輸出。只擷取「有多筆品名/數量/金額之類欄位」的那個表格本身。
4. 請以「儲存格」為單位逐一判斷（包含表頭儲存格），不要只看欄位標題或其他列的內容：只要某個儲存格包含非中文的文字說明（例如店名、地址、品名、付款方式、備註、表頭名稱等），請在該儲存格加上中文翻譯。
5. 翻譯格式必須嚴格遵守：「原文」+ 一個半角空格 + 一個半角斜線「/」+ 一個半角空格 + 「繁體中文翻譯」。斜線前後都一定要有空格，不可省略，也不可使用全角斜線「／」。正確範例：「아메리카노 / 美式咖啡」、「품목 / 品名」。錯誤範例（缺空格，禁止）：「아메리카노/美式咖啡」。
...
12. 若遇到跨頁，請自動合併。
13. 底部若有付款條件、稅金、總計等資訊，請整理在表格最下方的列，並把該資訊文字放在「品名／項目」欄，文字部分同樣依規則 4、5、6、7 處理；但「金額」欄請留空，不要填入小計、總計、含稅金額等任何數字，因為這些列是文字說明列，不是逐項商品，重複填入金額會導致與表格上方各項金額加總後重複計算。"""
