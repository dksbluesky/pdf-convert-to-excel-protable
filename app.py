...
1. 每一欄之間，請使用 "###" 作為分隔符號。
2. 每一列資料換一行，第一行必須是表頭。
3. 請以「儲存格」為單位逐一判斷（包含表頭儲存格），不要只看欄位標題或其他列的內容：只要某個儲存格包含非中文的文字說明（例如店名、地址、品名、付款方式、備註、表頭名稱等），請在該儲存格原文後面加上「 / 」，再接上繁體中文翻譯，例如：「아메리카노 / 美式咖啡」、「품목 / 品名」。
4. 特別注意：店名、分店名稱、付款方式（如信用卡、現金）等即使出現在看起來像「數值欄」的位置，仍然是文字內容，必須加上翻譯，不可因為欄位標題像數字欄而跳過。
5. 純數字、金額、日期、編號儲存格維持原樣即可，不需要翻譯，也不要加上斜線。
6. 不要新增任何欄位，輸出的欄位數量必須與原表格相同。
7. 不要輸出任何 Markdown 標記，只要純文字。
8. 金額請保留千分位符號，不要隨意移除。
9. 若遇到跨頁，請自動合併。
10. 底部若有付款條件、稅金等資訊，請整理在表格最下方的列，文字部分同樣依規則 3、4、5 處理。"""
